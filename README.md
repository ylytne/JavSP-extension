...
### 新增易用性功能
- **现代化用户界面**：使用前端技术栈实现了用户界面。
- **历史遗留问题处理工具**：
  针对原项目历史整理产生的历史遗留问题，在工作台中新增了批量维护工具：
...
- **外挂字幕同步归档**：自动识别同名字幕文件，一并归档并重命名。
- **docker部署支持**：双端架构原生支持（docker部署暂未高强度测试，暂不保证可用和稳定）
...
特别鸣谢原项目 [Yuukiy/JavSP](https://github.com/Yuukiy/JavSP) 奠定的基础。感谢原项目作者与贡献者们。
